...
func main() {
	...
	for {
		...
		/*
		   ...
		   fmt.Fprintln(os.Stderr, " op1 : ", op1)

		   fmt.Fprintln(os.Stderr, " op2 : ", op2)
		*/
		...
		*/
		...
		//	ang := p1.frontchpdiff(ax, ay)

		//  thinkk:=p1.modinifinte(p1.nextCheckPointId)
		...
		/*
		      lx1,ly1:=p1.nextchp()
		   if    p1.dist(lx1,ly1)<1200 {
		           altx,alty:=p1.altredchp()
		          finalx=lx1+altx
		          finaly=ly1+alty
		...
		*/
		...
		//	fmt.Fprintln(os.Stderr, " info : ", ang / 18)
		...
		/*
		           if tar.nextCheckPointId!=tar.pv.nextCheckPointId && tar.pv.nextCheckPointId==globalchn {
		               loc=false
		           }


		         nod:=idon{tar.nextCheckPointId,0,0,0,true,loc}
		           for ii:=0;ii<checkpointCount;ii++ {


		   		nod= p2.blockn(tar,nod)

		           loc=nod.lock
		           if !nod.fail{
		           	thrust2,finalx2, finaly2= nod.tthrust,nod.xx,nod.yy
		               break
		           }
		           }
		*/
		...
		/*
		          	cc:= cpi[nextring(tar.nextCheckPointId)]

		    angy,angx:=angletocord(tar.angle, 3000)

		    ang90x,ang90y:=angy,-1*angx
		    ang90xn,ang90yn:=-1*angy,angx

		    posangle:=Calcangle( ang90x,-1*ang90y)
		    negangle:=Calcangle( ang90xn,-1*ang90yn)

		    difpos:=difangli(posangle,cc)
		   difneg:=difangli(negangle,cc)

		   if Abs(difpos)<Abs(difneg) {

		       finalx2,finaly2= tar.x+ang90x,tar.y+ang90y
		   } else {
		        finalx2,finaly2= tar.x+ang90xn,tar.y+ang90yn
...
func predictcurve(p pod, cx, cy, t int) pod {
	...
	//thrust 100
	//fmt.Fprintln(os.Stderr, " difc : ",difc)
	...
	//	fmt.Fprintln(os.Stderr, " info : ", angle,angled,tan*180/math.Pi,d)

	//    tan+=8*math.Pi/180
	xcc := math.Cos(tan)
	ycc := math.Sin(tan)
	ycc = -1 * ycc
	disf := float64(Nspeed(p.speed()-(dfs+Heresyv), t))
	sdx, sdy := square(xcc, ycc, disf)

	p.pv = nil
	//np := pod{p.x + sdx, p.y + sdy, sdx, sdy, angletranf(p.angle + pa), p.nextCheckPointId}
...
angleopn:=Abs(op.anglerand(chn))
        ...
        ofx,ofy:=radtocord(angletorad(cpi[chn]), 1200)
        if op.rank!=1 {
        ofy,ofx=radtocord(angletorad(cpi[chn]), 1200)
...
succ:=pdist<500 ||  lock
attack:=opdistn<5000
rightang:=Abs(p.frontchpdiff(op.x,op.y))<16 && chn==op.nextCheckPointId

              fmt.Fprintln(os.Stderr, " nod is : ", opdistn,angleopn)
...
x,y=op.x,op.y
    ...
    lock=true
  fmt.Fprintln(os.Stderr, " succ is : ")
...
if pos && !succ {
    ...
    if anglep>16  {
                      fmt.Fprintln(os.Stderr, " anglep is : ")
